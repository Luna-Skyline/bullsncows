...
void loadPlayers(CnBplayer player[], int &numberOfPlayers) {
    ifstream inFile("playerData.txt");
    if (inFile.is_open()) {
        inFile >> numberOfPlayers;
        for (int i = 0; i < numberOfPlayers; ++i) {
            inFile >> player[i].isCPU >> player[i].CPUDifficulty;
            for (int j = 0; j < 4; ++j) {
                inFile >> player[i].numberCode[j];
            }
        }
        inFile.close();
    } else {
        cout << "Error opening player data file!" << endl;
    }
}
